...
vector<string> Population::getLabels(){
  return labels;
}
vector<int> Population::getValues(){
  return values;
}

void Population::operator % (Population& Z){
  vector<int> Zvalues = Z.getValues();
  for(int i = 0; i < this->values.size(); i++){
    this->values[i] = 100.0 * (float)this->values[i] / (float)Zvalues[i];
  }
}
...
std::string operator ^ (int n, Population& A){
    return A.labels[n];
}
